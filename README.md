...
视频详情与相册共用暗色沉浸工具栏和轻量圆形按钮。视频使用静态封面模糊背景，底部显示文件名与限宽播放控件；手机上下滑切换视频，照片仍左右滑，桌面均提供左右按钮。视频直连、HLS 播放和自动播放逻辑不变。
...
新转码的正式资产保存在 `Service/hls-cache/{原分类文件夹}/{视频ID}/{版本}/`；嵌套分类保留完整层级，根目录视频写入 `未分类/`。每个版本包含 `index.m3u8`、`init.mp4`、`seg_*.m4s`、封面和 `asset.json`。服务重启后从元数据恢复目录，不会重新转码已有内容；视频 ID、版本和播放 URL 不因资产整理而变化。该目录是清理源文件后的内容权威来源，必须纳入备份。

```text
Service/hls-cache/
├── 佩奇教你说英语 幼儿早教英语启蒙动画（全23集）【中英双语】/
│   └── {视频ID}/{版本}/...
├── 贝乐虎早教儿歌动画合集209课/
│   └── {视频ID}/{版本}/...
└── 未分类/
...
新版服务兼容旧的 `{视频ID}/{版本}/`，不会在启动时擅自搬动正式资产。旧资产需执行一次显式整理；工具只移动完整视频目录，不复制重转、不修改元数据、不合并、不覆盖。目标已存在、内容不完整或路径不安全时拒绝执行。

先使用包含本次修改的新版可执行文件做只读预览（本地在 `Service` 目录执行）：

```powershell
cargo run --locked -- --migrate-hls-folders ./hls-cache
```

生产整理顺序：构建新版 `bobo-learning-service:0.2.5` 镜像 → 停止旧 Service → 备份完整 `Service/hls-cache` 并核对备份 → 使用新版工具预览 → 执行整理 → 启动新版 Service。**不要在旧 Service 运行期间整理；旧版 Service 不能识别分类后的目录。** 本功能只涉及 Service，不要求重新发布 Web 或 Android。可先在仓库根目录执行 `docker build -f Service/Dockerfile -t bobo-learning-service:0.2.5 .` 准备镜像。

Compose 已指向包含本次修改的新版镜像时，在部署根目录执行：

```sh
docker compose stop service
# 在此完成整个资产库的独立备份并核对；不能仅备份元数据。
docker compose run --rm --no-deps service --migrate-hls-folders /app/hls-cache
docker compose run --rm --no-deps service --migrate-hls-folders /app/hls-cache --apply --service-stopped
docker compose up -d --no-build service
```

执行前后逐文件核验 SHA-256 和长度，持久化日志为资产库根目录的 `.hls-folder-migration-*.jsonl`；异常时尝试按相反顺序回移，禁止覆盖。进程被强制中断时可能新旧目录并存，新版服务仍可读取两种布局，停服后可再次预览继续整理。若需要回滚到旧 Service，必须先根据备份或整理日志恢复旧目录布局。日志和锁文件不要在任务运行时删除。

整理后可按分类整体管理。下架前先停止服务，将明确选中的分类目录移到 **`hls-cache` 之外的备份目录**，再启动服务、刷新客户端确认；保留备份比直接删除更安全。仅在资产库内部改分类名不受支持（分类名须与 `asset.json` 原路径一致）；不要把备份留在库内，也不要删除仍在转码的目录。在线资产变化同时由文件事件和默认 10 秒周期扫描同步；删除源文件已清理的正式资产无法自动恢复。
...
相册大图沿用打开时的文件顺序：手机左滑下一张、右滑上一张，桌面宽度达到 1024px 时显示左右按钮，也支持左右方向键；照片上下滑不切换，视频仍保留上下滑。双击或双指可放大照片，放大后拖动查看细节、还原后恢复左右翻页。详情页先显示缩略图，再渐入清晰原图；等待超过 8 秒提示慢网络，超过 30 秒显示失败并允许重试。当前原图就绪后只预取下一张，不批量下载整本相册。
